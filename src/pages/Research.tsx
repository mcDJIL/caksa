...
export default function Research() {
  ...
  return <PageHero label="Research & Projects" title={<>FLIGHT<br /><em>ARCHIVE.</em></>} description="Every CAKSA vehicle and research programme, collected in one evolving record." image={images.umar}>
    <section className="research-archive section-dark">
      ...
      <div className="archive-full-list">{visibleCards.map((card) => <article className="flight-record" key={card.number} role="button" tabIndex={0}><div className="flight-meta"><span>{card.number} / {card.year}</span><span className={card.status === "REALIZED" ? "status" : "status future-status"}>{card.status}</span></div><div className="flight-image h-[300px] max-[560px]:h-[210px] w-full overflow-hidden">{card.secondaryImage ? <div className="grid h-full w-full grid-cols-2 gap-1"><img className="h-full w-full object-cover" src={card.image} alt={`${card.name} primary vehicle`} /><img className="h-full w-full object-cover" src={card.secondaryImage} alt={`${card.name} remote-control unit`} /></div> : <img className="h-full w-full object-cover" src={card.image} alt={`${card.name} UAV research`} />}</div><div className="flight-content"><h3>{card.name}</h3><p>{card.type}</p>{card.secondaryImage && <span className="mb-2 inline-flex border border-orange px-2 py-1 font-mono text-[8px] uppercase tracking-[0.08em] text-orange">Paired control unit</span>}<small>{card.note}</small></div><span className="flight-arrow">↗</span></article>)}</div>
